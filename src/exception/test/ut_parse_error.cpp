...
TEST(ut_parse_error, string_initialization_constructor)
{
   const auto expected_what = std::string{ "xxx" };

   ore::sdk::parse_error exception{ expected_what };

   ASSERT_STREQ(expected_what.c_str(), exception.what());
}

TEST(ut_parse_error, cstring_initialization_constructor)
{
...
TEST(ut_parse_error, copy_constructor)
{
   const auto expected_what = std::string{ "xxx" };

   ore::sdk::parse_error exception_1{ expected_what };
   ore::sdk::parse_error exception_2{ exception_1 };

   ASSERT_STREQ(exception_1.what(), exception_2.what());
}

TEST(ut_parse_error, assignment_operator)
{
   const auto expected_what     = std::string{ "xxx" };
   const auto not_expected_what = std::string{ "yyy" };

   ore::sdk::parse_error exception_1{ expected_what };
   ore::sdk::parse_error exception_2{ not_expected_what };
...
TEST(ut_parse_error, set_and_get_cause)
{
   ...
   ore::sdk::parse_error exception{ "what" };
...
TEST(ut_parse_error, move_and_get_cause)
{
   const auto expected_cause_info =
      cause_info{ .source       = "source",
                  .file         = "file",
