...
TEST(ut_serialization_error, string_initialization_constructor)
{
   ...
   ore::sdk::serialization_error exception{ expected_what };

   ASSERT_STREQ(expected_what.c_str(), exception.what());
}

TEST(ut_serialization_error, cstring_initialization_constructor)
{
...
TEST(ut_serialization_error, copy_constructor)
{
   const auto expected_what = std::string{ "xxx" };

   ore::sdk::serialization_error exception_1{ expected_what };
   ore::sdk::serialization_error exception_2{ exception_1 };

   ASSERT_STREQ(exception_1.what(), exception_2.what());
}

TEST(ut_serialization_error, assignment_operator)
{
   const auto expected_what     = std::string{ "xxx" };
   const auto not_expected_what = std::string{ "yyy" };

   ore::sdk::serialization_error exception_1{ expected_what };
   ore::sdk::serialization_error exception_2{ not_expected_what };
